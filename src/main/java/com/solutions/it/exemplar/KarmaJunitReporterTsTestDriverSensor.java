 ...
 * based on the xml report generated by the karma-junit-reporter so that the
 * unit test report data can be shown in the dashboard.
 */
public class KarmaJunitReporterTsTestDriverSensor implements Sensor, Extension {

	private static final Logger LOG = LoggerFactory.getLogger(KarmaJunitReporterTsTestDriverSensor.class);
	private static final String TS_EXTENSION = "." + TypeScriptLanguage.KEY;
	...
	public KarmaJunitReporterTsTestDriverSensor(final FileSystem fileSystem, final Settings settings) {
	...
	public boolean shouldExecuteOnProject(final Project project) {
		...
		if (!isConfiguredToRun) {
			LOG.debug("{} will not execute since \"{}\" configuration was not found.",
					KarmaJunitReporterTsTestDriverSensor.class.getSimpleName(), KarmaJunitReportPlugin.REPORTS_PATH);
		}
	...
	protected void collect(final SensorContext context, final File reportsDir) {
		...
		new AbstractSurefireParser() {
			...
			protected Resource getUnitTestResource(final String classKey) {
				...
				if (classKey.endsWith(TS_EXTENSION)) {
	...
	private Map<String, String> getTestSuiteToFileNameMapping() {
		...
		if (StringUtils.isNotEmpty(settings.getString("sonar.karma.tests"))) {
			testDirPrefix = new StringBuilder().append(settings.getString("sonar.karma.tests"))
	...
	/**
	 * For a SureFire class name like "PhantomJS_1_9_8_(Mac_OS_X_0_0_0).myFile",
	 * this method will strip off the first qualifier
	 * "PhantomJS_1_9_8_(Mac_OS_X_0_0_0)" if found and return "myFile".
	 * 
	 * @param className
	 *            the SureFire class name
	...
	/**
	 * Returns a java.io.File for the given path. If path is not absolute, returns a
	 * File with project base directory as parent path.
	 */
